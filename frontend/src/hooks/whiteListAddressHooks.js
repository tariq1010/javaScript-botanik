...
} from "store/redux/slices/addressesSlice";
...
export const AddWhiteListAddressesHook = () => {
  ...
  const addAddresses = async () => {
    ...
    try {
      if (!token) {
        ...
        const merkleRoot =
          await new WhitelistAddressService().generateMerkleRoot(data);
        ...
        if (!merkleRoot?.data?.error) {
          ...
          merkleRoot.data.status === 505 &&
            openNotification("Error", "Error creating root", "error");
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    saveAddresses.result &&
      dispatch(resetWhiteList()) &&
      openNotification("Added", "Addresses added to whiltelist", "success");
    if (saveAddresses.error) {
      saveAddresses.status == 401 && navigate("/admin-login");
      saveAddresses.status == 505 &&
        openNotification("Error", "Somthing went wrong", "error");
    }
...
export const GetProofHook = () => {
  const { getProof } = useAppSelector((state) => state.addresses);
